...
import { isObject, isFunction, extend, addComma, isUnset, randStr, toFixed, encodeJSON, isSet, isString, debounce, groupBy } from 'mytoolkit';
...
function axisX(chart) {
  ...
  var axisLabelSetting = extend({}, defaultOptions.axisLabel, xAxis.axisLabel || {}); // axis label
...
function axisY(chart) {
  ...
  var axisLabelSetting = extend({}, defaultOptions.axisLabel, yAxis.axisLabel || {}); // axis label
...
function drawGradient(chart, color, defaultColor) {
  ...
  if (isObject(color)) {
    var colorString = encodeJSON(color);
    ...
    var r = '',
        id = 'lc-gradient-' + randStr(6);

    if (color.type === 'linear') {
      color.colorStops.forEach(function (item) {
        r += "<stop offset=\"".concat(toFixed(item.offset, 0, 100), "%\" stop-color=\"").concat(item.color, "\"/>");
      });
...
function drawLine(chart, layer, s, index) {
  var emitter = chart.emitter,
      ...
      scaleY = chart.scaleY,
      scaleX = chart.scaleX;
  var lineStyle = extend({}, defaultOptions.lineStyle, s.lineStyle || {});
  ...
  var areaStyle = extend({}, defaultOptions.areaStyle, s.areaStyle || {});
  ...
  var plotStyle = extend({}, defaultOptions.plot, s.plotStyle || {});
  var currentPlotGroup;

  if (plotStyle.show) {
    currentPlotGroup = plotGroup.safeSelect("g.lc-plot-group-".concat(index));
    var plotSetting = extend({}, defaultOptions.plot, s.plot || {});
...
function drawBar(chart, layer, s, index) {
  ...
  var barStyle = extend({}, defaultOptions.barStyle, s.barStyle || {});
...
function drawPie(chart, layer, s, index) {
  ...
  var focusAnimation = isSet(s.focusAnimation) ? s.focusAnimation : true;
  ...
  _createClass(chart, [{
    ...
    value: function calculateStackData() {
      ...
      types.forEach(function (type) {
        ...
        var stackGroups = groupBy(chartsByStack, 'stack');
